...
function parseMinecraftColors(motd) {
    if (!motd) return "";
    ...
    parts.forEach((part, index) => {
        // Skip empty fragments generated by leading § symbols
        if (!part) return;
        ...
        // Color codes reset active formatting in Minecraft
        if (colorMap[code]) {
...
async function updateStatus(ip, port) {
    try {
        ...
        if (data.online) {
            ...
            versionEl.innerHTML = parseMinecraftColors(data.version);
            motdEL.innerHTML = parseMinecraftColors(data.motd);
            playersEl.innerText = `${data.players} / ${data.max}`;
            if (data.playerList.length > 0) {
                ...
                data.playerList.forEach(playerName => {
                    ...
                    li.innerHTML = parseMinecraftColors(playerName);
